...
description: Find top Instagram recruiting accounts, follow them, engage with one post, and reply with a high-energy message.
metadata:
  openclaw:
    requires:
      browser: openclaw
...
Run focused outbound engagement on Instagram.
...
- Use `--message` only for `replied`, with the exact posted comment text.
...
```bash
node scripts/engagement.mjs engaged-users --platform instagram --db memory/engagement.db
```
...
Search Instagram for `volleyball recruit` using `skills/instagram-browser`. Collect account usernames from results. Skip any username already in the exclusion list from Step 1. Pick the first 3 valid accounts.
...
```bash
node scripts/engagement.mjs create --platform instagram --username "<username>" --post-url "<profile_url>" --action followed --db memory/engagement.db
```
...
2. Click the Like (heart) button.
3. Take a snapshot. Confirm the heart icon is filled/red or the button reads "Unlike".
4. If confirmed, log it:
```bash
node scripts/engagement.mjs create --platform instagram --username "<username>" --post-url "<post_url>" --action liked --db memory/engagement.db
```
5. If not confirmed after 2 retries, skip logging and move to 3c.

#### 3c — Comment on that post

1. On the same post from 3b, click the comment field.
2. Type a short, high-energy, on-topic comment.
3. Submit it.
4. Take a snapshot. Confirm your comment text appears in the comments section.
5. If confirmed, log it:
```bash
node scripts/engagement.mjs create --platform instagram --username "<username>" --post-url "<post_url>" --action replied --message "<exact_comment_text>" --db memory/engagement.db
```
